...
export async function proxy(request: NextRequest) {
  ...
  if (isProtected && !user) {
    ...
    // headed (including any query string, e.g. UTMs — client-side attribution
    // capture reads it back out of `next` on the signup page, see
    // src/lib/analytics/consent.ts) so they land back on it after the
