...
import { ARTICLES, metadataFor, SITE_PAGES } from "@/lib/seo/registry";

export const metadata: Metadata = metadataFor(SITE_PAGES.blog);

const ARTICLE_LIST = Object.values(ARTICLES);
        ...
        <section>
          ...
          <div className="max-w-[820px] mx-auto px-4 grid gap-4">
            {ARTICLE_LIST.map((article, i) => (
              <Reveal key={article.path} delay={i * 80}>
                <Link
                  href={article.path}
                ...
                >
                  <p className="font-mono text-[10.5px] font-medium uppercase tracking-[0.14em] text-[#94A3B8] mb-2.5">
                    Reviewed{" "}
                    {new Date(`${article.lastModified}T12:00:00Z`).toLocaleDateString("en-GB", {
                      month: "long",
                      year: "numeric",
                    })}
                  </p>
                  <h2 className="font-display text-lg font-semibold text-[#0F2044] mb-2 leading-snug">{article.shortTitle}</h2>
                  <p className="text-sm leading-relaxed text-[#475569] mb-4">{article.excerpt}</p>
